...
namespace Group_Project_Draft
{
    public partial class Sports : Form
    {
        ...
        private void SetLacrosseTheme()
        {
            ...
            // Clear the event box and show a loading message
            eventBox.Items.Clear();
            eventBox.Items.Add("Loading Lacrosse schedule...");

            // Show the DataGridView and load data
            dgvLacrosseSchedule.Visible = true;
            LoadLacrosseSchedule();
        }
        private void LoadLacrosseSchedule()
        {
            string connectionString = "Data Source=UserDatabase.db;Version=3;"; 

            using (SQLiteConnection conn = new SQLiteConnection(connectionString))
            {
                string query = "SELECT GameDate, HomeTeam, AwayTeam, GameTime, Location FROM LacrosseSchedule ORDER BY GameDate";

                using (SQLiteDataAdapter adapter = new SQLiteDataAdapter(query, conn))
                {
                    DataTable dt = new DataTable();
                    adapter.Fill(dt);
                    dgvLacrosseSchedule.DataSource = dt;
                }
            }
        }
        ...
        {
            Profile profileForm = new Profile(currentUsername); // Pass logged-in username
